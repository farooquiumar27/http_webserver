...
void Request::setInt(const char *_str,int num)
{
this->intMap[string(_str)]=num;
}
int Request::getInt(const char  *_str)
{
...
if(extensionEquals(resource+lastDotIndex+1,"sct"))
{
mimeType=(char *)malloc(sizeof(char)*10);
strcpy(mimeType,"text/html");
}
...
void Request::findAndReplace(const char *resource)
{
this->sct=1;
...
{
found=0;
for(unordered_map<string,string>::iterator it=valueMap.begin( );it!=valueMap.end( );++it)
{
if(strncmp(arr.c_str( ),it->first.c_str( ),it->first.size( ))==0)
{
for(int i=0;i<it->second.size( );i++)fputc(it->second[i],t);
found=1;
break;
}
}
